...
from reader.ocr.vietocr.tool.translate import process_image
from reader.ocr.vietocr.tool.create_dataset import createDataset
from reader.ocr.vietocr.tool.translate import resize


class OCRDataset(Dataset):
    def __init__(self, lmdb_path, root_dir, annotation_path, vocab, image_height=32, image_min_width=32,
                 image_max_width=512, transform=None):
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        max_w = max([i['img'].shape[2] for i in batch])
        max_h = batch[0]['img'].shape[1]
        new_img = np.zeros((batch_size, 3, max_h, max_w)) + 1
        for i, _img in enumerate(img):
            new_img[i, :, :, :_img.shape[2]] = _img
        img = np.array(new_img, dtype=np.float32)

        tgt_input = np.array(tgt_input, dtype=np.int64).T
        tgt_input = np.ascontiguousarray(tgt_input)
        tgt_output = np.roll(tgt_input, -1, 0).T
        tgt_output[:, -1] = 0
        tgt_output = np.ascontiguousarray(tgt_output)
