...
from reader.ocr.vietocr.model.beam import Beam
from reader.ocr.vietocr.model.transformerocr import VietOCR
from reader.ocr.vietocr.model.vocab import Vocab
...
def translate(img, model, max_seq_length=128, sos_token=1, eos_token=2, debug=False):
    "data: BxCXHxW"
    model.eval()
    device = img.device
    ...
    with torch.no_grad():
        ...
        while max_length <= max_seq_length and not all(np.any(np.asarray(translated_sentence).T == eos_token, axis=1)):
            ...
            output, memory, attention = model.transformer.forward_decoder(tgt_inp, memory)
        ...
        if debug:
            for a in attentions:
                plt.imshow(a, aspect='auto', origin='lower', interpolation='none')
                plt.show()
...
def process_image(image, image_height, image_min_width, image_max_width):
    ...
    img = cv2.resize(image, (new_w, image_height), interpolation=cv2.INTER_AREA)
