...
class DBProcess(object):
    ...
    def process_batch(self, images):
        assert self.resize_type == 0
        assert len(images) > 0
        batch_size = len(images)
        n_channel = images[0].shape[2]
        original_shapes = []
        resize_imgs = []
        ratios = []
        max_h, max_w = -100, -100
        for img in images:
            ...
            resize_img, ratio = self.resize_image_type0(img)
            resize_img = self.normalize(resize_img)
            max_h, max_w = max(max_h, resize_img.shape[1]), max(max_w, resize_img.shape[2])
            ratios.append(ratio)
            resize_imgs.append(resize_img)

        batch_img = np.zeros((batch_size, n_channel, max_h, max_w), np.float32)
        for i, resize_img in enumerate(resize_imgs):
            batch_img[i, :, :resize_img.shape[1], :resize_img.shape[2]] = resize_img
